...
const _SHUFFLE_H_TOTAL_SIZE =
	_SHUFFLE_H_SEED_SIZE +
	_SHUFFLE_H_ROUND_SIZE +
	_SHUFFLE_H_POSITION_WINDOW_SIZE;
...
function isLte<T>(left: T, right: T, message: string): void {
	if (!(left <= right)) {
		throw new Error(
			`${message || "Expected value to be lte"}: ${left} <= ${right}`,
		);
	}
}
...
function innerShuffleList(
	...
	isLte(
		seed.length,
		_SHUFFLE_H_SEED_SIZE,
		`seed length is not lte ${_SHUFFLE_H_SEED_SIZE} bytes`,
	);
	...
	while (true) {
		...
		const pivot =
			Number(bytesToBigInt(h.subarray(0, 8)) % BigInt(listSize)) >>> 0;
...
export function naiveComputeSyncCommitteeIndicesElectra(
	...
	while (syncCommitteeIndices.length < SYNC_COMMITTEE_SIZE) {
		const shuffledIndex = computeShuffledIndex(
			i % activeValidatorCount,
			activeValidatorCount,
			seed,
		);
		const candidateIndex = activeValidatorIndices[shuffledIndex];
		const randomBytes = digest(
			Buffer.concat([seed, intToBytes(Math.floor(i / 16), 8, "le")]),
		);
		const offset = (i % 16) * 2;
		const randomValue = bytesToInt(randomBytes.subarray(offset, offset + 2));

		const effectiveBalanceIncrement =
			effectiveBalanceIncrements[candidateIndex];
		if (
			effectiveBalanceIncrement * MAX_RANDOM_VALUE >=
			MAX_EFFECTIVE_BALANCE_INCREMENT * randomValue
		) {
